    ...
    <div className="flex flex-col h-full justify-start items-center bg-gradient-to-tr from-blue-500/20 to-sky-400/20 relative">
      <div className='z-50 block w-full justify-center items-center bg-sky-500 text-white p-4 '>
        <div className="flex justify-between items-center">
          <h1 className="w-full font-black uppercase tracking-tighter">ENS Delegates</h1>
        ...
        <div className="flex flex-col w-full space-y-4 items-center p-2 bg-sky-50/20;">
          <div className="flex items-center space-x-8 w-full px-8">
            <div className="tracking-wider font-medium text-2xl flex flex-col w-2/5">
              ...
              <p className="tracking-tighter font-semibold mt-4">Voting Power: <span>{vp}</span></p>
            </div>
            <form className="flex w-full bg-stone-50 rounded-md p-2 border"
          ...
          <Marquee gradientColor={[8, 30, 48]} className='rounded-full border-2 border-sky-800/40 items-center overflow-clip' speed={24}>
            {names.map((d, i) =>
          ...
          <div className="grid grid-cols-4 xl:grid-cols-6 p-4 h-full overflow-y-scroll bg-inherit">
            ...
            {logs.map((log, i) => (
              <div className="flex flex-col p-1">
                {(log.args.newBalance > log.args.previousBalance) ? (
                  <div className="bg-green-500 shadow-lg p-2 rounded-md flex-col w-full cursor-pointer h-full items-center hover:scale-[.98] hover:shadow-lg hover:opacity-80 border-2 hover:animate-pulse">
                    <div className=" flex w-full space-x-2 items-center ">
                      <div className="justify-center items-center flex">
                        <a href={`https://etherscan.io/tx/${log.transactionHash}`} className="bg-indigo-400 text-white h-6 w-6 rounded-full justify-center items-center" target="_blank">
                          <p className="text-center text-xs font-black mt-[4px]">{[i + 1]}</p>
                    ...
                    <div className="flex w-full mt-4 justify-between">
                      <p>Balance:</p>
                      <p className="truncate w-1/3">{ethers.utils.formatUnits(log.args.newBalance._hex)}</p>
                    </div>
                    <div className="flex w-full  justify-center items-center">
                      <p className="truncate w-1/3 font-bold">{(log.args.newBalance > log.args.previousBalance) ? ('+') : ('-')}{(ethers.utils.formatUnits(log.args.newBalance._hex) - ethers.utils.formatUnits(log.args.previousBalance._hex))}</p>
                ...
                ) : (<div className="bg-red-500 shadow-lg  text-white p-2 rounded-md flex-col w-full cursor-pointer h-full items-center hover:scale-[.98] hover:shadow-lg hover:opacity-80 border-2 hover:animate-pulse">
                  <div className=" flex w-full space-x-2 items-center ">
                    <div className="justify-center items-center flex">
                      <a href={`https://etherscan.io/tx/${log.transactionHash}`} className="bg-indigo-400 text-white h-6 w-6 rounded-full justify-center items-center">
                        <p className="text-center text-xs font-black mt-[4px]">{[i + 1]}</p>
                      </a>
                    </div>
                    <div className="truncate">{log.transactionHash}</div>
